...
```
   Host (RISC-V) ──descr──▶ ┌────────── Command Engine ──────────┐
                            │  W↓     W↓      W↓      W↓         │
                            ▼        ▼        ▼        ▼
                        CONV Eng  Requant   EWE Eng  POOL Eng
                            │        │        │        │
                            └──────  L1_Manager  ──────┘
                                  │           │
                         L1Mesh 2M SRAM     UDMA
...
- **資料流**：DRAM (LPDDR6) ⇄ UDMA ⇄ L1_Manager ⇄ {L1Mesh SRAM, 4 Engines}。
- 4 條 compute pipeline 共用一顆 L1_Manager 作為 on-chip 記憶體仲裁中心。
...
| Mem ctrl | **L1_Manager** | 中央仲裁 | 4 Engine + UDMA + L1Mesh SRAM 之間的 traffic arbiter |
...
- Requant / EWE / POOL：保留 AXI_R + AXI_W，partial sum / output 透過 L1_Manager 仲裁進出 L1Mesh。
...
| Bits | 欄位 | 說明 |
|---|---|---|
| [4] | `op_class` | enum：CONV / Requant / EWE / POOL / UDMA + 3 reserved |
...
依 `op_class` 解出五種 body：CONV / Requant / EWE / POOL / UDMA。
詳細欄位定義：
...
6. **UDMA** 在背景把上一個 tile 的結果寫回 DRAM、預取下一個 tile（double-buffer `[TBD]`）。

> 4 個 compute engine 之間的次序、是否能 chain 為 fused layer、partial sum 是否常駐 L1Mesh — 全部 `[TBD]`。
