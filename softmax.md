...
| **C (INT8)** — INT8 chain | systemc/include/mdla7/{descriptor.h,ewe_pool.h} systemc/src/mdla7_model_runner.cpp | **done** — added `ES_DEQUANT_INT8` (11) and `ES_QUANT_FP_INT8` (12) subtypes. EweEngine dequant casts `(int8 - zp_in) -> fp16` (input scale dropped — softmax-invariant); quant casts `clamp(round(fp*256) + zp_out)` (TFLite softmax convention). Runner emits dequant→FP16 chain→quant for INT8 softmax inputs. Also gates the `try_stream_binary_ewe_softmax` pre-pass on the flag so the OK_SOFTMAX dispatch runs the chain instead of the streamed-fused monolithic ES_SOFTMAX. bmm_softmax_bmm_int8 cx: EWE 2→81 tasks (1 MUL + 16 rows × 5), POOL 0→32 tasks (16 rows × 2). BMM fast 9/13 = baseline (no new fails). |
...
3. ~~INT8 chain via dequant/requant wrap~~ — done. INT8 softmax now runs the chain when `MDLA7_DECOMPOSE_SOFTMAX=1` via the new `ES_DEQUANT_INT8` / `ES_QUANT_FP_INT8` EWE subtypes. Input scale is dropped (softmax-invariant); output uses the TFLite convention (scale=1/256, zp=-128). The `try_stream_binary_ewe_softmax` pre-pass is gated on the flag so the OK_SOFTMAX dispatch can see the layer.
