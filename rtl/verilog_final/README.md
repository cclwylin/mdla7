...
./rtl/batch/run_verilog_final.py --filter slice --rerun-all \
  --crc-coverage --conv-sram-window-commands 1024 --conv-sram-window-count 5
```
...
`refB/sramB`. Layers with no final descriptor are reported as `SKIP`. The runner
summary also prints suite totals for `refcrc`, `sramcrc`, `refB`, and `sramB`.
