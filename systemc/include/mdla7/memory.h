...
private:
    ...
    void reserve_route(uint32_t src_x, uint32_t src_y,
        ...
        while (x != dst_x) {
            ...
            sc_core::sc_time start = reserve_resource(
                mesh_router_out_finish_[node_id(x, y)][out_dir], now, beats,
                stats_.router_wait_ns, stats_.router_service_ns);
            ready = max_time(ready, start);
            if (east) {
                start = reserve_resource(mesh_hlink_finish_[y][x][0], now, beats,
                                         stats_.link_wait_ns, stats_.link_service_ns);
                ready = max_time(ready, start);
                ++x;
            } else {
                start = reserve_resource(mesh_hlink_finish_[y][x - 1][1], now, beats,
                                         stats_.link_wait_ns, stats_.link_service_ns);
                ready = max_time(ready, start);
                --x;
            }
        ...
        while (y != dst_y) {
            ...
            ready = max_time(ready, start);
            if (south) {
                start = reserve_resource(mesh_vlink_finish_[y][x][0], now, beats,
                                         stats_.link_wait_ns, stats_.link_service_ns);
                ready = max_time(ready, start);
                ++y;
            } else {
                start = reserve_resource(mesh_vlink_finish_[y - 1][x][1], now, beats,
                                         stats_.link_wait_ns, stats_.link_service_ns);
                ready = max_time(ready, start);
                --y;
            }
        ...
        sc_core::sc_time start = reserve_resource(
            mesh_router_out_finish_[node_id(x, y)][DIR_LOCAL], now, beats,
            stats_.local_wait_ns, stats_.local_service_ns);
        ready = max_time(ready, start);
    }

    // Mesh mode: 16 SRAM banks sit on a 4x4 mesh. The original model charged
    ...
    void impose_mesh_latency(sc_core::sc_time bank_finish[N_BANKS],
        ...
        for (uint32_t burst_off = offset; burst_off < end; ) {
            ...
            if (!optimistic) {
                ...
                for (unsigned side = 0; side < 2; ++side) {
                    for (unsigned row = 0; row < MESH_H; ++row) {
                        const sc_core::sc_time finish = mesh_edge_finish_[rw][side][row];
                        const sc_core::sc_time start =
                ...
                ingress_ready = reserve_resource(
                    mesh_edge_finish_[rw][best_side][best_row], burst_now,
            ...
            for (uint32_t bank = 0; bank < N_BANKS; ++bank) {
                ...
                sc_core::sc_time t = service_sram_beat(bank_finish[bank], ready, chunk);
    ...
    sc_core::sc_time mesh_edge_finish_[2][2][MESH_H];
