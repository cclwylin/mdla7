#!/usr/bin/env python3
"""Regression sweep over model/ETHZ_V6/*.tflite — emit CSV (pattern, ms).
...
Usage:
    python3 systemc/run_ethz_v6.py
    python3 systemc/run_ethz_v6.py --model-dir <path>    # override input dir
    python3 systemc/run_ethz_v6.py --csv <path>          # override output csv
    python3 systemc/run_ethz_v6.py --filter mobilenet    # substring filter
"""
...
def main():
    ...
    ap.add_argument("--model-dir", default=str(REPO_ROOT / "model" / "ETHZ_V6"))
    ap.add_argument("--csv",       default=str(OUT_DIR / "ethz_v6_regression.csv"))
    ...
    print(f"==== ETHZ_V6 regression: {len(models)} models ====")
    rows = []
    t_total = time.time()
