#!/usr/bin/env python3
"""Regression sweep over model/ETHZ_v5/*.tflite — emit CSV (pattern, ms).
...
flow, parses the simulator's "sim time: NNNN ns" line, and writes:
    ...
    output/ethz_v5_regression.csv
        pattern,ms,status

where `pattern` is the .tflite stem and `ms` is sim cycles / 1.9e6
(spec frequency = 1.9 GHz). Failed compiles (unsupported op, missing buffer,
…) emit a blank ms and a short status note so you can see what's still gating
coverage.

Usage:
    python3 systemc/run_ethz_v5.py
    python3 systemc/run_ethz_v5.py --model-dir <path>    # override input dir
    python3 systemc/run_ethz_v5.py --csv <path>          # override output csv
    python3 systemc/run_ethz_v5.py --filter mobilenet    # substring filter
"""
...
def main():
    ...
    ap.add_argument("--model-dir", default=str(REPO_ROOT / "model" / "ETHZ_v5"))
    ap.add_argument("--csv",       default=str(OUT_DIR / "ethz_v5_regression.csv"))
    ...
    print(f"==== ETHZ_v5 regression: {len(models)} models ====")
    rows = []
    t_total = time.time()
