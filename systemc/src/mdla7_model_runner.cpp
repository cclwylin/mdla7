...
// v13: INT8 softmax decomp entry/exit casts. Dequant reads INT8, writes FP16
// (input dtype byte = DT_INT8x8 so EweEngine picks the int path; subtype
// distinguishes dequant from regular int ops). Quant inverts that: reads FP16,
// writes INT8. zp is stored inline in EweBody.scalar_imm.
Descriptor make_ewe_dequant_int8(uint32_t in_addr, uint32_t out_addr,
                                 uint32_t rows, uint32_t K, int16_t zp_in,
                                 uint8_t wait_a, uint8_t signal_tag) {
    Descriptor d{};
    d.hdr.op_class_subtype = OC_EWE;
    d.hdr.dtype = DT_INT8x8;
    ...
    e.n = 1;  e.h = uint16_t(rows);  e.w = 1;  e.c = uint16_t(K);
    e.scalar_imm = zp_in;
    e.subtype = ES_DEQUANT_INT8;
    return d;
}

Descriptor make_ewe_quant_to_int8(uint32_t in_addr, uint32_t out_addr,
                                  uint32_t rows, uint32_t K, int16_t zp_out,
                                  uint8_t wait_a, uint8_t signal_tag) {
    Descriptor d{};
    d.hdr.op_class_subtype = OC_EWE;
    d.hdr.dtype = DT_INT8x8;
...
int sc_main(int argc, char* argv[]) {
    ...
    auto is_attention_softmax_meta = [](const LayerMeta& S) {
        // Accept both [H>1, W, C] (multi-head layout) and [H=1, W_flat, C]
        // (queries×heads flattened into W, as generated by the tile-softmax
        // generator for the bmm_softmax_bmm stress test).
        const bool attention_matrix =
            (S.in_h >= 1 && S.in_h <= 32) &&
            (S.in_w >= 1) &&  // uint16_t; any width accepted for attention
            (S.in_c > 1 && S.in_c <= 2048) &&
            (S.in_h > 1 || S.in_w >= 64);  // exclude H=W=1 classifier softmaxes
    ...
    // fc(bmm) score-tile → {EWE binary / RESHAPE}* → attention SOFTMAX.
    // When the chain ends at an attention softmax and the score tile fits in L1,
    // suppress DRAM for every layer on the path (fc(bmm) + any EWE/RESHAPE
    // intermediaries) so softmax reads the score directly from L1.
    // This is the key optimisation for the bmm_softmax_bmm tile stress test:
    // a 128 KB score tile stays on-chip instead of spilling 128 MB to DRAM.
    for (uint32_t k = 0; k < N; ++k) {
        if (!is_fc_kind(metas[k].op_kind)) continue;
        const auto& fc = metas[k];
        // Walk forward through transparent layers toward softmax
        uint32_t j = k + 1;
        while (j < N) {
            const auto& M = metas[j];
            if (is_attention_softmax_meta(M)) break;
            if (M.op_kind == OK_RESHAPE || M.op_kind == OK_MUL ||
                M.op_kind == OK_ADD    || M.op_kind == OK_SUB)
                ++j;
            else
                break;
        }
        if (j >= N || !is_attention_softmax_meta(metas[j])) continue;
        // Score tile must fit in L1 (in_size or ref_size check)
        const uint64_t score_bytes = uint64_t(fc.out_h) * fc.out_w * fc.out_c
                                     * (fc.dtype == DT_FP16 ? 2 : 1);
        if (score_bytes > L1_BUDGET / 4) continue;  // too large; keep DRAM path
        for (uint32_t m = k; m < j; ++m)
            producer_no_store[m] = true;
    }
    ...
    // Post-override: re-apply fc(bmm) score-tile → softmax L1 chain.
    // Handles a RUN of consecutive same-shape fc(bmm) slices (one per head)
    // that collectively produce the score tensor consumed by a softmax chain.
    // The binary-EWE force-false above clears individual slice suppression;
    // re-asserting here (after all overrides) keeps the score in L1 instead
    // of spilling 128 KB × N_slices to DRAM for the tile stress test.
    {
        uint32_t k = 0;
        while (k < N) {
            if (!is_fc_kind(metas[k].op_kind)) { ++k; continue; }
            // Find the end of this contiguous fc(bmm) run (same output shape).
            uint32_t run_end = k;
            while (run_end + 1 < N &&
                   is_fc_kind(metas[run_end + 1].op_kind) &&
                   metas[run_end + 1].out_h == metas[k].out_h &&
                   metas[run_end + 1].out_w == metas[k].out_w &&
                   metas[run_end + 1].out_c == metas[k].out_c &&
                   metas[run_end + 1].dtype  == metas[k].dtype)
                ++run_end;
            // Walk forward from run_end+1 through EWE/RESHAPE to find softmax.
            uint32_t j = run_end + 1;
            while (j < N) {
                if (is_attention_softmax_meta(metas[j])) break;
                if (metas[j].op_kind == OK_RESHAPE ||
                    metas[j].op_kind == OK_MUL     ||
                    metas[j].op_kind == OK_ADD     ||
                    metas[j].op_kind == OK_SUB)
                    ++j;
                else
                    break;
            }
            if (j < N && is_attention_softmax_meta(metas[j])) {
                const uint32_t n_slices   = run_end - k + 1;
                const uint32_t elem_bytes = (metas[k].dtype == DT_FP16) ? 2 : 1;
                const uint64_t score_bytes =
                    uint64_t(n_slices) * metas[k].out_h * metas[k].out_w
                    * metas[k].out_c * elem_bytes;
                if (score_bytes <= L1_BUDGET / 4) {
                    // Mark entire fc(bmm) run + EWE/RESHAPE chain as in-L1.
                    for (uint32_t m = k; m < j; ++m)
                        producer_no_store[m] = true;
                }
            }
    ...
    for (uint32_t i = 0; i < N; ++i) {
        ...
        auto try_stream_binary_ewe_softmax = [&]() -> bool {
            ...
            // v13: when softmax decomposition is enabled the OK_SOFTMAX
            // dispatch needs to see the softmax layer to emit the 5-op chain
            // (POOL_MAX → SUB → EXP → POOL_SUM → DIV, bracketed by
            // DEQUANT/QUANT casts on INT8 inputs). This streamed pre-pass
