...
int sc_main(int argc, char* argv[]) {
    ...
    for (uint32_t i = 0; i < N; ++i) {
        ...
        if (L.op_kind == OK_TRANSPOSE && i + 2 < N &&
            ...
            if (pref_wgt_l1 >= align64(uint32_t(cur_working)) && pref_top <= L1_BUDGET) {
                flush_pending();
        ...
        auto try_stream_conv_ewe = [&]() -> bool {
            ...
            if (A.dtype != B0.dtype) return false;
            ...
            const bool pool_tail = [&]() -> bool {
                ...
                if (Q.dtype != P.dtype) return false;
                if (Q.in_h != P.out_h || Q.in_w != P.out_w || Q.in_c != P.out_c)
                    return false;
                if (Q.out_h != Q.in_h || Q.out_w != Q.in_w || Q.out_c != Q.in_c)
                    return false;
                if (Q.k_h != 1 || Q.k_w != 1 || Q.s_h != 1 || Q.s_w != 1)
                    return false;
                if (Q.p_t || Q.p_b || Q.p_l || Q.p_r)
                    return false;
                if (graph_metas) {
                    const auto& GP = graph_metas[ewe_end];
            ...
            for (uint32_t oh_done = 0; oh_done < A.out_h; oh_done += tile_oh, ++tile_id) {
                ...
                const uint32_t dram_out_off = oh_done * A.out_w * A.out_c * elem;
                Microblock mb{};
                mb.id = tile_id;
                ...
                if (pool_tail) {
                    ...
                    acc[pool_tail_idx].sram_w += tile_out_bytes;
                    ++pool_count_so_far;
                ...
                if (d2s_tail) {
                    ...
                    const uint8_t st_tag = alloc_tag();
                    emit_stream(make_udma(final_addr, metas[final_layer].dram_out + dram_out_off,
                                          tile_out_bytes, /*dir*/ 1, st_tag, final_done),
                                mb, final_layer,
                                SMF_STORE | (is_last_h ? SMF_FINAL_TILE : 0));
                    acc[final_layer].sram_r += tile_out_bytes;
                    acc[final_layer].dram_w += tile_out_bytes;
                    ++udma_count_so_far;
        ...
        auto try_stream_ewe_conv = [&]() -> bool {
            ...
            auto slot_bytes_for = [&](uint32_t toh) {
                const uint32_t worst_ewe_rows = std::min<uint32_t>(H, toh + (padded_input ? 2u : 0u));
            ...
            const uint32_t max_ewe_rows = std::min<uint32_t>(H, tile_oh + (padded_input ? 2u : 0u));
            ...
            for (uint32_t oh_done = 0; oh_done < H; oh_done += tile_oh, ++tile_id) {
                ...
                const uint32_t ewe_lo = (padded_input && oh_done > 0) ? (oh_done - 1) : oh_done;
                const uint32_t ewe_hi = padded_input ? std::min<uint32_t>(H, oh_hi + 1) : oh_hi;
        ...
        auto try_stream_binary_ewe_chain = [&]() -> bool {
            ...
            const bool unary_tail = [&]() -> bool {
                ...
                if (GP.consumer_count != 1 ||
                    ...
                    GP.last_consumer_layer  != int32_t(unary_tail_idx))
                    return false;
                if (GU.producer0_layer != int32_t(end))
                    return false;
                if (U.dtype != P.dtype) return false;
                if (U.in_h != P.out_h || U.in_w != P.out_w || U.in_c != P.out_c)
                    return false;
