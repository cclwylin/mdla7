...
| Regression / Report | 怎麼知道改 code 後有沒有壞？怎麼看模型時間和 profile？ | [`systemc/run_model.py`](../systemc/run_model.py)、[`systemc/run_mdla6_pattern.py`](../systemc/run_mdla6_pattern.py) |
...
| 階段 | 做什麼 | 產物 |
|---|---|---|
| model lookup | `run_model.py` 找到符合 pattern 的 `.tflite` | model path |
| compile | parse TFLite flatbuffer、建立 layer metadata、準備 DRAM blob、算 reference | `output/<stem>.bin` |
...
這是 `run_model.py` 預設檢查。它能證明 compiler reference 與 simulator 對同一套 MDLA7 semantics 一致。
...
| Day | 任務 | 成果 |
|---|---|---|
| 1 | 跑 `run_model.py inception_v3_quant`，打開 profile HTML | 知道工具怎麼跑 |
...
| 誤解 | 正確理解 |
|---|---|
| `run_model.py` 是 simulator | 它是 driver，真正 simulator 是 `test_model` binary |
...
接下來第 1 章會進入最實用的第一步：把 repo 跑起來，理解 `run_model.py` 的三個階段、輸出檔案、profile HTML，以及最基本的 regression 命令。
...
| [`systemc/`](../systemc) | SystemC simulator、compiler scripts、regression scripts、model output |
...
| [`systemc/scripts/`](../systemc/scripts) | TFLite compiler、validator、plotter |
...
新人一開始最常進的目錄是 `systemc/`，因為大部分 build、run、regression 都在那裡。
...
| 檔案 | 你要知道的事 |
|---|---|
| [`systemc/run_model.py`](../systemc/run_model.py) | 單模型入口：build、compile、simulate、report |
...
| [`systemc/scripts/gen_model_profile.py`](../systemc/scripts/gen_model_profile.py) | 產生 regression index HTML |
| [`systemc/run_mdla6_pattern.py`](../systemc/run_mdla6_pattern.py) | 跑 MDLA6 pattern table 對照 |
| [`systemc/run_ethz_v6.py`](../systemc/run_ethz_v6.py) | 跑 ETHZ v6 model sweep |
...
如果你只記一件事：`run_model.py` 是入口，`compile_model.py` 是 compiler，`test_model.cpp` 是 simulator driver，`include/mdla7/*.h` 是硬體 module。
...
`run_model.py` 會自動先跑 make，所以平常你可以直接跑 model。但 junior 第一天建議先手動跑一次 `make -s`，確認 C++ / SystemC 環境 OK。
...
python3 run_model.py unet_int16
```
...
```text
[step 1/3] make
[step 2/3] compile inception_v3_quant.tflite -> output/inception_v3_quant.bin
...
```text
test_model: output/inception_v3_quant.bin  (126 layers, v3, ...)
...
summary: 1/1 ran
csv:  output/mdla6_pattern_regression.csv
html: model_profile.html
```
...
| `run_mdla6_pattern.py` | 跑 MDLA6 baseline 對照表 | regression CSV、model_profile index |
| `run_ethz_v6.py` | 跑 ETHZ v6 collection | ETHZ sweep CSV |
...
| 檢查 | 成功長相 |
|---|---|
| `make -s` | 產生或更新 `build/test_model` |
| `--list` | 能列出 model |
| `run_model.py inception_v3_quant` | summary 顯示 0 FAIL |
| pattern regression | row 結尾是 `ok` |
| output HTML | `output/inception_v3_quant.html` 存在且可打開 |
...
| report index | `systemc/model_profile.html` | 若 regression 更新可一起 commit |
| output artifact | `systemc/output/...` | 多半 gitignored，不一定 commit |
...
| 現象 | 可能原因 | 排除方向 |
|---|---|---|
| `python3 run_model.py xxx` 找不到模型 | pattern 不對或 model 不存在 | 先跑 `--list` |
...
```bash
head -5 output/inception_v3_quant.profile.csv
```
...
本章把 repo 跑起來的基本路徑建立起來：`make` build simulator，`run_model.py` 跑單模型，`run_mdla6_pattern.py` 跑 regression，`output/*.html` 看 profile。從下一章開始，我們會回到硬體架構本身，先讀 `spec/spec.md`，把 Host、Command Engine、五個 engines、UDMA、L1、DRAM 的關係建立清楚。
...
```bash
cd systemc
python3 run_ethz_v6.py --filter imdn_quant
```
...
csv_path = Path("output/imdn_quant.profile.csv")
...
make
python3 run_ethz_v6.py --filter imdn_quant
python3 run_ethz_v6.py --filter imdn_float
python3 run_ethz_v6.py --filter resnet_quant
```
...
for m in ["imdn_quant", "imdn_float", "resnet_quant"]:
    rows = list(csv.DictReader(open(f"output/{m}.profile.csv")))
