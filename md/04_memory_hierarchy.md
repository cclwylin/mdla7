...
L1Mesh AXI input probe 可用環境變數打開：

```bash
MDLA7_L1_AXI_PROBE=batch/output/l1mesh_axi_probe.csv ./systemc/build/test_model ...
```
...
前 16 欄對應 read AXI input lane 0..15，後 16 欄對應 write AXI input
...
| mesh conflict | `--l1-timing=mesh` | 逐 bank shared 1R/W SRAM service + dual-4x4 transparent NoC contention reservation；長 access 依 AXI 16-beat burst 切成 4096B aggregate chunks；適合找 lane imbalance / queue buildup。 |
